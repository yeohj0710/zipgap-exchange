  ...
  return (
    <div className="panel flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--color-line)] px-3 py-2">
        <span className="text-[13px] text-[var(--color-mute)]">최근 체결</span>
        <span className="text-[12px] text-[var(--color-dim)]">시각 / 값 / 수량</span>
      </div>
      <div className="flex-1 overflow-y-auto">
        {trades.length === 0 ? (
          ...
          trades.map((t) => (
            <div
              key={t.id}
              className="grid grid-cols-[auto_1fr_auto] items-center gap-2 px-3 py-[3px] text-[13px]"
            >
